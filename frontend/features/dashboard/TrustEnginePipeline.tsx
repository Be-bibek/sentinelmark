  ...
  return (
    <div className="bg-white/5 border border-white/10 rounded-2xl p-6 relative overflow-hidden group">
        ...
        {PIPELINE_STAGES.map((stage, i) => (
          <React.Fragment key={stage.id}>
            <div className="flex flex-col items-center gap-3 shrink-0">
              <div className="w-12 h-12 rounded-xl bg-black/40 border border-white/10 flex items-center justify-center shadow-lg relative group-hover:border-white/20 transition-colors">
            ...
            {i < PIPELINE_STAGES.length - 1 && (
              <div className="flex-1 min-w-[30px] flex items-center justify-center relative shrink-0 px-2">
                ...
                />
                <ArrowRight className="w-3 h-3 text-zinc-600 relative z-10 bg-[#0c0c0c]" />
              </div>
            )}
          </React.Fragment>
        ))}
      ...
      <div className="mt-6 flex justify-between items-center text-xs border-t border-white/5 pt-4">
        ...
        <span className="text-zinc-500">Last Output: <span className="text-white font-bold">{decision}</span></span>
      </div>
    </div>
  );
}
